...
public interface OINVRepository extends JpaRepository<OINV, Integer> {
	...
	/*@Query("SELECT T0.docEntry,T0.docNum, T0.docDate, T0.docDueDate, T0.cardCode, T0.cardName, T0.docTotal, T0.docStatus FROM OINV T0 WHERE T0.cardCode = ?1 "
			+ "AND  T0.docDueDate >=?2 AND T0.docDueDate <=?3 and T0.docStatus IN (?4) ORDER BY T0.docStatus DESC")
	List<OINV> selectByCustCodeAndDates(String custCode, Date toDate, Date fromDate, List<String> docStatusList);*/

	@Query("SELECT new OINV(T0.docEntry,T0.docNum, T0.docDate, T0.docDueDate, T0.cardCode, T0.cardName, T0.docTotal, T0.docStatus) FROM OINV T0 WHERE T0.cardCode = ?1 "
			+ "AND  T0.docDueDate >=?2 AND T0.docDueDate <=?3 and T0.docStatus IN (?4) ORDER BY T0.docStatus DESC")
	List<OINV> selectByCustCodeAndDates(String custCode, String fromDate, String toDate, List<String> docStatusList);
